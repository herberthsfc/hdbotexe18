			 ...
			 if (isGroup && isLevelingOn) {
				...
				try {
					...
					if (requiredXp <= getLevelingXp(sender)) {
						...
						await reply(`*「 LEVEL UP 」*\n\n➸ *Nome*: ${sender}\n➸ *XP*: ${getLevelingXp(sender)}\n➸ *Level*: ${getLevel} -> ${getLevelingLevel(sender)}\n\nPor ser ativo, seu nivel aumentou!! 🎉🎉`)
					}
				} catch (err) {
					console.error(err)
				}
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'audio':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Digite o que quer que eu diga!', text, {quoted: mek})
                                   /*if (!isDaftar) return reply(mess.only.daftarB)*/
	    ...
	    case 'kapankah':
			...
			case 'fechargp':
					...
					const close = {
					text: `[ ❗ ]  O grupo foi fechado pelo adm @${nomor.split("@s.whatsapp.net")[0]}\nagora só os adms podem falar!`,
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `[ 💎 ]  O Grupo foi aberto pelo adm @${sender.split("@")[0]}\nagora todos os membros podem falar!`,
					contextInfo: { mentionedJid: [sender] }
					}
